...
Train / Dev(Validation) / Test, realized as 5 seeded, stratified-per-class partitions (40/15/15/15/15, no class leakage). The "Test touched once" rule is applied PER BLOCK, not to one fixed partition: at progressive stage k, Train = P0..P(k-1) and Test = Pk — the next 15% block still unseen. We score Pk exactly once while unseen (an honest generalization estimate), then fold it into Train and never reuse it as a clean test. Dev/tuning + error analysis run only on the current Train (engine's early-stop holdout) and already-seen blocks, never on the block currently acting as Test. The hidden grader test is the ultimate one-shot test. This is a deliberate, documented mapping (split_data.py docstring spells it out) — the split logic must NOT be silently changed.
...
Model = hand-built small ResNet (ResNet-18 shape), self-contained in model.py. Stem (7x7 s2 conv → BN → ReLU → 3x3 s2 maxpool) → 4 residual stages [64→128→256→512] → AdaptiveAvgPool2d(1) → Dropout(0.4) → Linear(512,20), ~11.2M params. Adaptive pool accepts any input (train small, eval 224). Two components are syllabus-EDGE and must be defended in the interview, NOT hidden: (a) BatchNorm after each conv — a normalization layer like the LayerNorm we saw after each Transformer attention block, adapted to conv channels; running mean/var ride in the state_dict and predict.py's .eval() uses them, so inference is batch-size-independent. GroupNorm is the lower-risk fallback if an examiner pushes back. (b) Residual / skip connections y = f(x)+x (He 2016, taught in the Transformers unit as the vanishing-gradient fix) — same mechanism, here applied to convs, which is what lets us train this depth from random init. Both buy easier optimization + usable depth; the extra width buys lower bias. This is why the ResNet beats the old plain 4-block VGG (no norm, no skips, ~1.2M params, under-parameterized). 🚩 Hard-to-undo: model.py must never import a local helper — grader breaks otherwise. 🚩 predict.py builds ModelArchitecture() with NO args then load_state_dict, so the DEFAULT ctor args (base=64, blocks (2,2,2,2), num_classes=20) define the loaded architecture — train.py must match them; only `dropout` is a no-args-shape-safe knob (Dropout holds no parameters), everything else needs weights regenerated.
...
Augmentation = OFFLINE materialized twins (make_augmented.py writes augmented JPGs to dataset/train_aug/), torchvision/PIL only, zero network. Originals pass through the deterministic eval transform as-is; all variety comes from pre-baked, seeded-per-file twins on disk (byte-for-byte reproducible across all 4 teammates, and viewable for error analysis). Default copies=2 (~40k twins). Filters are weighted (name, fn, weight) and sampled WITHOUT replacement: rotation=4 and color_jitter=4 (the only CONFIRMED real test manipulations, from dataset/augmentations/), grayscale/salt_pepper/blur=2, invert=1 (label-risky: yellow lemon→blue, so kept rare). Mean-fill rotation (corners filled with the image's own mean colour) is our nearest in-bounds proxy for "the background changed." BACKGROUND SWAP is deliberately omitted — it needs segmentation + external background assets, which violates train-from-scratch/no-external-data and isn't taught. The whole approach is in-syllabus data augmentation (taught on MNIST: elastic distortions / training-set expansion). No albumentations (extra dep), no wandb (phones home → breaks no-network rule). augment.py's build_train_transform (online path) is kept as a documented fallback but is DEAD in the default offline-only path.
...
Person A — Architecture (model.py): own ModelArchitecture; small ResNet-18 (residual blocks + BatchNorm + GAP + head Dropout 0.4, ~11.2M params), tune depth/width/regularization. Stays self-contained, [B,20] logits. dropout is the only state_dict-safe lever to retune from error analysis.
...
Person C — Augmentation / robustness (augment.py, make_augmented.py): the differentiator — offline weighted twin generation (rotation + colour jitter weighted heaviest, the confirmed test axes) + OOD stress probes. Owns the 50% OOD score half. Guided by error analysis.
...
Stage 4 — Robustness (C+D): Person C adds offline twin augmentation; Person D adds robust_eval (clean-Dev vs each provided-OOD set, plus a single combined SCORE = 0.5·clean + 0.5·mean(OOD), mirroring the grader's 50/50 weighting). Retrain. Goal: SCORE climbs — hold clean accuracy while OOD accuracy climbs; optimize SCORE, not clean acc alone. Review: clean vs OOD vs SCORE before/after.
...
Robustness: robust_eval.py prints clean-Dev vs each provided-OOD set and the combined SCORE (0.5·clean + 0.5·mean OOD); the clean→OOD gap shrinks and SCORE rises after Stage 4.
