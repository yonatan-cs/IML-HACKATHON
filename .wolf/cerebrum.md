...
- [2026-06-26] SUBMITTED model = ResNet-18 + Squeeze-Excitation (~11.28M, BatchNorm, dropout 0.4), AdamW lr1e-3 wd1e-2 + cosine + label-smoothing 0.1 + grad-clip 1.0 + early-stop. Result: held-out clean 0.847, SCORE 0.906 (OOD-mean ~0.96 > clean). Robustness via OFFLINE weighted twins (rotate4/color4/gray2/sp2/blur2/invert1, mean-fill rotation, no bg-swap). KNOWN GAP: submitted challenge2_.../train.py is a collapsed self-contained version using ONLINE torchvision aug + random 15% split — differs from README/dev offline-twin progressive pipeline that actually trained the weights. README also names engine.py/SGD-selectable/train-at-128 not in submitted code. Weak class: acoustic_guitar vs laptop/mobile_phone.
- [2026-06-26] CONFIRMED via sha+mtime: submitted weights.joblib is byte-identical to submissions/my_team/weights.joblib (sha fe20eb7d…) = the OFFLINE-TWIN gated model (train_stage4.csv Jun25 22:17 → 0.847). The challenge2_.../train.py that shipped was written Jun26 09:33 — a simplified ONLINE-aug rewrite that did NOT train the weights and contradicted the README. FIX: rewrote submission train.py as self-contained OFFLINE-twin script (inlined PIL filters rotate/color/gray/sp/blur/invert weights 4/4/2/2/2/1, mean-fill, materialize_twins, stratified split, AdamW/cosine/ls0.1/clip/early-stop) → now matches README + weights. Cleaned README (dropped engine.py + SGD-selectable mentions). weights.joblib UNCHANGED. Lesson: when packaging, the shipped train.py must be the script that actually made the weights, not a fresh simplification.
